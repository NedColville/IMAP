...
def getData(i, trange): 
    ...
    if i==4:
        ...
        temp=pyspedas.themis.fgm(trange,notplot=True,varnames=['thc_fgs_gse'],level='l2')
        start_datetime = datetime.strptime(trange[0], '%Y-%m-%d/%H:%M:%S').timestamp()
        end_datetime = datetime.strptime(trange[1], '%Y-%m-%d/%H:%M:%S').timestamp()
        #hist, bins, _=plt.hist(np.diff(np.array(temp['thc_fgs_gse']['x'])))
        data.append(temp)
...
def dlFormat(trange,fname):
    headers=["Time","vx","vy","vz","x","y","z","Bx","By","Bz"]
    ...
    #themTrange= [manipulate_datetime(trange[0], 40*60),manipulate_datetime(trange[1], 40*60)]
    #themData=getData(3,themTrange)
    themData=getData(3,trange)
    themData=((themData[1]['thc_pos_gse']['x'],themData[1]['thc_pos_gse']['y'].T[0]),
              (themData[1]['thc_pos_gse']['x'],themData[1]['thc_pos_gse']['y'].T[1]),
              (themData[1]['thc_pos_gse']['x'],themData[1]['thc_pos_gse']['y'].T[2]),
              (themData[0]['thc_fgs_gse']['x'],themData[0]['thc_fgs_gse']['y'].T[0]),
              (themData[0]['thc_fgs_gse']['x'],themData[0]['thc_fgs_gse']['y'].T[1]),
              (themData[0]['thc_fgs_gse']['x'],themData[0]['thc_fgs_gse']['y'].T[2]))
    #adjusted_datetime_list = [dt + timedelta(seconds=40*60) for dt in time]
    #themData=np.array(multiInterp(adjusted_datetime_list,False,*themData)).T
    ...
    dfThem=pd.DataFrame(themData).interpolate(method='linear',limit=8)
    
    if dfWind.isna().any().any():
        check=1
        print("Wind data faulty")
    elif dfACE.isna().any().any():
        check=1
        print("ACE data faulty")
    elif dfDSC.isna().any().any():
        check=1
        print("DSCOVR data faulty")
    elif dfThem.isna().any().any():
        check=1
        print("THEMIS data faulty")
    if check==0:
        ...
        headers=['Time','x','y','z','Bx','By','Bz']
        dfThem.to_csv(fname+"ARTEMIS.csv",header=headers, index=False)
...
temp=dlFormat(['2018-9-19/08:00:00','2018-9-19/11:00:00'],"test")
...
"""
#%%
trange=['2018-11-27/00:00:00','2018-11-27/05:00:00']
