...
#print(magnitude_differences)
#distances are in form (0,1),(0,2),(1,2)
#i.e. ACE and Wind, ACE and DSCOVR and Wind and DSCOVR
...
for i in range(len(times)):
    ...
    for data in dataset:
        dfFilt = data[(data['Time'] >= times[i][0]) & (data['Time'] <= times[i][1])]
        ts=np.mean(np.array(dfFilt['x'])/np.array(dfFilt['vx']))
        tempDs.append(ts)
...
#%%
plt.hist(ts/60)
plt.xlabel("Average Propagation Time to Magnetopause, minutes")
plt.ylabel("Number")
...
ACEvals=np.load('AvsR_zvCCs.npy')
OMNIVals=np.load('OvsR_zvCCs.npy')

plt.hist([multi,ACEvals])
plt.xlabel("Cross Correlation in SYM/H")
plt.ylabel("Number")
plt.title("Performance Comparison")
plt.legend(["Weighted Average", "ACE"])
...
plt.show()
#%%
windvals=np.load('WvsR_zvCCs.npy')
dscvals=np.load('DvsR_zvCC.npy')
plt.hist([ACEvals,windvals,dscvals,multi])
plt.xlabel("Cross Correlation in SYM/H")
...
plt.show()
multiMax=np.load("MvsR_maxCCs.npy")

binPlot(abs(deltaT/60),multiMax,"Absolute Time Shift Errors, mins", "Max CC", "Maximum CC against Absolute Shift Error",20)
plt.grid(alpha=0.5)
#%%
